...
export class AppComponent {
  ...
  cart : Array<{quantity: number, book: Book}> = [
  ...
  addCart(book : Book){
    let idx = this.cart.findIndex((ele) => ele.book.ISBN == book.ISBN)

    if (idx == -1){
      this.CartValue += Math.floor(book.price.value)
      this.cart.push(
        {
          quantity: 1,
          book   
  ...
  removeCart(event: {quantity: number, book: Book}){
    let idx = this.cart.findIndex((ele) => ele.book.ISBN == event.book.ISBN)
